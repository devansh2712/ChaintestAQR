								...
								<div className="row">
									<div className="col-lg-12 ">
										<div className="row header_text px-4 py-2 m-0 d-flex justify-content-between">
											<div>
												<h5 className="text-center mt-1">{ t("comman.users")}</h5>
											</div>
											<div>
											...
											<div className="table-responsive custom_table">
												<table className="table  table-sm border table-bordered text-center">
													<thead className="thead-light">
														<tr>
															<th scope="col">{ t("comman.id")} </th>
															<th scope="col">{ t("comman.firstName")} </th>
															<th scope="col">{ t("comman.email")}</th>
															<th scope="col">{ t("comman.action")}</th>
														</tr>
													</thead>
													<tbody>
														{ 
																...
																data.map((item, index) => {
																	return (
																		<tr key={item._id}>
																			...
																			<td>{ item.email }</td>
																			<td>{ t("comman.status")}</td>
																			<td>
																				<div className="btn-group">
																					<button
																						type="button"
																						className="btn btn-primary btn-1 btn-sm"
																						onClick={() =>  this.props.history.push("/user/view/"+item._id)}
																					>
																						{ t("comman.view")}
																					</button>
																					<button
										...
										<div className="ml-2 my-3 pb-4 bottom-btn">
											<Link className="btn btn-primary px-4" to="/user/add">
...
export default withTranslation("common")(connect(mapStateToProps, {getUserList})(Listing));