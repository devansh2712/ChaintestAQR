...
/// property approvel list

export const getApprovelList = (data) => async dispatch => {
	return new Promise(async (resolve, reject) => {
		const response = await aqarChain.get("/api/token/approvellist");
...
export const getApprovelDetails = (data) => async dispatch => {
	return new Promise(async (resolve, reject) => {
		const response = await aqarChain.get("/api/token/approvellist/"+data);
		console.log("response", response);
		if (response.status === 1) {
			resolve(response);
		} else {
...
export const changeInitializationStatus = (data, id) => async dispatch => {
	return new Promise(async (resolve, reject) => {
		const url = `/api/token/${id}`;
		const response = await aqarChain.put(url, data);
		console.log("response", response);
		if (response.status === 1) {
			resolve(response);
		} else {
